...
export const CustomWalletButton = () => {
  ...
  // Function to fetch account info with rate limiting
  const fetchAccountInfo = async () => {
    ...
    try {
      ...
      toast.error("Failed to fetch account info. Please try again later.");
    }
  };

  // Debounced version of fetchAccountInfo
  const debouncedFetchAccountInfo = debounce(fetchAccountInfo, 2000);

  useEffect(() => {
    if (!connection || !publicKey) return;
  
    const accountChangeListenerId = connection.onAccountChange(
      publicKey,
      (updatedAccountInfo) => {
        ...
        {!publicKey ? (
          ...
          <div className="relative" ref={dropdownRef}>
            <motion.button
              whileHover={{ scale: 0.9 }}
              whileTap={{ scale: 0.8 }}
              className="flex bg-black text-[12px] md:text-[18px] font-bold border-double border-2 border-red text-white h-[40px] md:h-[60px] w-[200px] rounded-xl items-center"
            ...
            <motion.div
              ...
              className="absolute bg-black text-white border-double border-2 border-red font-bold text-center text-[16px] w-[200px] mt-1 rounded-md z-50 py-2"
            >
              <motion.button
        ...
        {open && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-65 ">
            <motion.div
              ...
              className="relative bg-black py-4 px-10 rounded-xl max-w-[450px] w-full border-2 border-red"
            >
              <div className="flex flex-col gap-4">
                ...
                {wallets.map((wallet) => (
                  <motion.button
                    ...
                    className="flex bg-red bg-opacity-50 items-center py-4 px-6 text-white w-full border-2 border-white rounded-xl"
                  >
                    <Image
